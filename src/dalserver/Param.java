...
/**
 * The Param (parameter) class is used to store a runtime parameter instance,
 * e.g., a GET/POST request parameter input to a DAL service.
 *
 * This is a minimal version of the parameter mechanism and does not
 * attempt to be fully general, or serve cases other than service request
 * parameters.  Our focus here is on a runtime parameter instance with
 * minimal metadata.
 *
...
public class Param {
     ...
     */
    public Param(String name, EnumSet<ParamType> type, String value,
     ...
     */
    public Param(String name, EnumSet<ParamType> type, String descr)
	throws DalServerException {
     ...
     */
    public Param(String name, EnumSet<ParamType> type, String value,
	String descr) throws DalServerException {
     ...
     * @param	name	The parameter name.
     * @param	value	The default value of the parameter.
     * @param	level	The parameter level (see ParamLevel)
     *
     * This constructor is used to define non-standard string parameters,
     * particularly system parameters (level=ParamLevel.SERVICE).  
     */
    public Param(String name, String value, ParamLevel level)
	throws DalServerException {

	this(name, EnumSet.of(ParamType.STRING), value, level, false, null);
    }
    ...
    /**
     * Simple constructor to create a new copy of an existing parameter.
     *
     * @param	param	Some existing parameter.
     *
     * A new copy of all parameter data is created, but attributes like the
     * parameter name, type, etc. are merely referenced.
     */
	...
	this.isSet = param.isSet;
	this.description = param.description;

	this.setValue(new String(param.value));
    }
	...
	if (isRangeList) {
		...
		if (this.value != null) {
			...
			this.value.equalsIgnoreCase("y") ||
			this.value.equalsIgnoreCase("1")) ? true : false;
		} else
		    this.value_b = false;

		// Update the string value.
		this.value = this.value_b ? "true" : "false";
		break;
