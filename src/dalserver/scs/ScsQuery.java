...
public class ScsQuery extends DbmsQuery {
	...
	// *** MJF 092915
        // Get an optional name of the schema to be used.
	// *** MJF 092915
        schemaName = params.getValue("schemaName");
        if (schemaName == null)
            schemaName = "%";
	...
	// *** MJF 092915
	// Get the MAXREC value from the parameter set so we can enforce
	// it in the request respose.
	// *** MJF 092915
        p = params.getParam("maxrec");
        if (p != null) {
            maxrec = p.intValue();
	...
	try {
	    ...
	    // *** MJF 092915
            // Get the requested schema (if any)
	    // *** MJF 092915
...
*/
	    ...
	    // *** MJF 092915
	    // Increase the fetch size and set direction to speed up result.
	    // *** MJF 092915
