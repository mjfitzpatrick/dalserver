...
/**
 * ScsMySql provides a simple cone search (SCS) capability for any
 * catalog which can be accessed via JDBC to a MySQL database.
 *
 * @version	1.0, 24-Aug-2007
 * @author	Doug Tody
 *
 * This file is obsolete and is replaced by the generic ScsQuery.
 * To be deleted later.
 */
public class ScsMySql {
    /** Connection to the remote DBMS. */
    private Connection conn;

    /** Tasble Configuration. */
    private ConfigTable conf;

    /** Constructor to generate a new MySql query object, providing
     * the functionality to query a remote MySQL-hosted catalog.
     */
    public ScsMySql(String jdbcDriver) throws DalServerException {
	try {
	...
	try {
	    ...
	    this.conf = new ConfigTable(params, null);
	...
	if (args.length == 0 || args[0].equals("hms")) {
	    ScsMySql mysql;
	    try {
		mysql = new ScsMySql("com.mysql.jdbc.Driver");
