...
public class RequestResponse {
    ...
    // Text, VOTable and TSV/CSV.  "Native" format is supported as a pass-through.
	...
	System.out.println ("In RequestResponse.writeCsv() ...\n");
    ...
    /**
     * Create a TSV representation of the RequestResponse object and
     * write it to the given OutputStream.
     *
     * <p>Table fields (nonscalar TableParams) which have a non-null TSV
     * value are output to create the TSV.  The TSV value has two elements
     * formatted as "index;label".  Index specifies the TSV column
     * order.  Label is either an explicit column label, or "@col"
     * where "col" is the name of the column of the data model to be
     * used for the TSV column label.  The data model specifies the
     * default set of fields to be output for the TSV format, however
     * the default fields or labels may be overriden by setting the TSV
     * attributes of the table fields in the RequestResponse.
     * See {@link dalserver.TableParam#setCsvKeyword}.
     *
     * @param	out	OutputStream to which the VOTable is to be written.
     */
    @SuppressWarnings("unchecked")
    public void writeTsv(OutputStream out) throws DalServerException {
	final int MAXCOLS = 1024;
	String[] fieldKeys = new String[MAXCOLS];
	String[] fieldLabels = new String[MAXCOLS];
	int nfields=0, maxIndex=0;
	...
	System.out.println ("In RequestResponse.writeTsv() ...\n");

	// Check whether the table has TSV field tags.
	for (Iterator i = fields.entrySet().iterator();  i.hasNext();  ) {
	    Map.Entry me = (Map.Entry) i.next();
	    obj = (Object) me.getValue();
	    if (obj == lastObj)
	...
	// Get a list of the TSV columns and their labels.
	for (Iterator i = fields.entrySet().iterator();  i.hasNext();  ) {
	    Map.Entry me = (Map.Entry) i.next();
	    obj = (Object) me.getValue();
	    if (obj == lastObj)
