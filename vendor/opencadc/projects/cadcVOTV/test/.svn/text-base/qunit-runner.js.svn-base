	...
	startQunit: function ()
  {
		...
		var requirements = function (pkg)
    {
      ...
      for (var key in reqts.scripts)
      {
        ...
        {
          var nextScript = reqts.scripts[key];
          var scriptToRun;

          if (nextScript.indexOf(qunitRunner.fs.separator) == 0)
          {
            scriptToRun = nextScript;
          }
          else
          {
            scriptToRun = path + nextScript;
          }

          console.log("Injecting: " + scriptToRun);

          try
          {
            phantom.injectJs(scriptToRun);
          }
          catch (e)
          {
